    ...
    const handleVerifyResetCode = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response && response.data.token) {
                localStorage.setItem('resetToken', response.data.token); // Stocker le token dans le localStorage
                history.push("/resetPwd"); // Rediriger vers la réinitialisation du mot de passe
            }
        } catch (error: any) {
